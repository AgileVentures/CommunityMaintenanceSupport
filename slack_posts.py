import glob, re, json, csv, datetime,time,pytz, functools
    ...
    with open('./av/stripe_customers.csv') as stripe_customers:
        ...
        with open('./av/unresolved_emails.csv') as ur_emails:
            ...
            for unresolved_email in reader:
                resolved_email = list(filter(lambda h: h == unresolved_email['unresolved_email'],unresolved_emails))
...
def find_slack_id_by_slack_name(users, user_name):
    resolved_id = list(filter(lambda id: users[id]['name'] == user_name, users))
